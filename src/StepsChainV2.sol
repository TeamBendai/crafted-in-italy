...
contract StepsChainV2 {
    ...
    constructor(string memory _producerName, string memory _producerStepName, uint256 _startTime, uint256 _endTime, string memory _description, string memory _additionalData, string memory _hiddenData) {
        ...
        steps[genesisStepHash] = Step(_producerName, _producerStepName, _startTime, _endTime, _description, _additionalData, _hiddenData, 0);
    ...
    function addStep(string memory _producerName, string memory _producerStepName, uint256 _startTime, uint256 _endTime, string memory _description, string memory _additionalData, string memory _hiddenData, bytes32 _previousStep) public returns (bytes32) {
        require(_previousStep == 0 || steps[_previousStep].startTime != 0, "Invalid previous step");
        ...
        steps[transactionHash] = Step(_producerName, _producerStepName, _startTime, _endTime, _description, _additionalData, _hiddenData, _previousStep);
